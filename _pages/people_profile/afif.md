---
layout: people
title: Afif Al Mamun
permalink: /people/afif/profile/
subtitle: PhD Student, University of Calgary

profile:
  align: right
  image: https://afif.me/assets/img/prof_pic-800.webp
  image_circular: false # crops the image to make it circular
  more_info: >
    <p>520B ICT Building</p>
    <p>2500 University Drive NW</p>
    <p>Calgary, AB, Canada T2N 1N4</p>

selected_papers: true # includes a list of papers marked as "selected={true}"
social: true # includes social icons at the bottom of the page

announcements:
  enabled: false # includes a list of news items
  scrollable: true # adds a vertical scroll bar if there are more than 3 news items
  limit: 5 # leave blank to include all the news in the `_news` folder

latest_posts:
  enabled: false
  scrollable: true # adds a vertical scroll bar if there are more than 3 new posts items
  limit: 3 # leave blank to include all the blog posts

social_links:
  email: afif.mamun@ucalgary.ca
  github_username: afifaniks
  linkedin_username: afifaniks
  scholar_userid: XuauQRIAAAAJ
  website: https://afif.me
---

I am Afif, a graduate student at the <a href='https://ucalgary.ca'>University of Calgary</a>, supervised by <a href='https://giasuddin.ca/'>Dr. Gias Uddin</a> and working with <a href='https://casweb.59b0587b.public.multi-containers.ibm.com/ibm/cas/canada/'>IBM CAS Canada</a>. I also serve as a Graduate Assistant in Research & Teaching at UCalgary. Alongside my studies, I am currently working as an intern at <a href='https://ibm.com'>IBM Canada</a> through <a href='https://www.mitacs.ca/'>Mitacs Accelerate</a>. My current research centers on Large Language Models and Software Engineering, areas I am deeply passionate about. Before my graduate studies, I worked for several years as a Software Engineer specializing in AI. During this time, I had the opportunity to contribute to various cutting-edge projects across different domains. I was also honored with a gold medal upon receiving my Bachelor of Science degree.

## Awards & Honors

- **Alberta Graduate Excellence Scholarship (AGES)**  
  *University of Calgary*, 2025  

- **ESE Graduate Research Award**  
  *University of Calgary*, 2025  

- **ACM SIGSOFT Distinguished Paper Award**  
  *ACM International Conference on the Foundations of Software Engineering (FSE)*, 2025  
  Awarded for the paper *“Hallucination Detection in Large Language Models with Metamorphic Relations.”*

- **IBM CAS Project of the Year Award**  
  *IBM Canada*, 2024  

- **Alberta Graduate Excellence Scholarship (AGES)**  
  *University of Calgary*, 2024  

- **ESE Graduate Research Award**  
  *University of Calgary*, 2024  

- **50 Years Victory of Bangladesh Gold Medal**  
  *Daffodil International University*, 2022  

- **Professor Dr. M. Lutfar Rahman Scholarship**  
  *Daffodil International University*, 2017  


## Publications & Research Contributions

- **Hallucination Detection in Large Language Models with Metamorphic Relations**  
  *Yang Borui, **Md Afif Al Mamun**, Jie Zhang, Gias Uddin*  
  Proceedings of the ACM International Conference on the Foundations of Software Engineering (FSE), 2025.  
  🏆 *ACM SIGSOFT Distinguished Paper Award*.  
  This work introduces metamorphic relations to systematically detect hallucinations in LLMs, advancing reliability analysis for AI-assisted software engineering.

- **Evaluating Deep Learning Based Domain Generalization for Motion Mitigation in Multi-Center Brain MRI**  
  *Saad Ashraf, **Md Afif Al Mamun**, Mumu Akter, Roberto Souza, Mariana Bento*  
  Reconstruction and Imaging Motion Estimation (RIME), 2025.  
  Investigates domain generalization strategies to improve robustness of motion mitigation models across multi-center MRI datasets.

- **Neonatal Brain MRI Motion Correction using Adult MRI**  
  *Saad Bin Ashraf, **Md Afif Al Mamun**, Mumu Akter, Roberto Souza, Mariana Bento*  
  International Society for Magnetic Resonance in Medicine (ISMRM), 2024.  
  Explores transfer learning approaches for motion correction in neonatal MRI using adult MRI data.

- **TLNet: A Deep CNN Model for Prediction of Tomato Leaf Diseases**  
  *Md Afif Al Mamun, Dewan Ziaul Karim, Subroto Nag Pinku, Tasfia Anika Bushra*  
  International Conference on Computer and Information Technology (ICCIT), 2020.  
  Proposes a convolutional neural network for accurate plant disease detection from leaf images.

- **Bangla Music Genre Classification Using Neural Network**  
  *Md Afif Al Mamun, Imamul Kadir, AKM Shahariar Azad Rabby, Abdullah Al Azmi*  
  International Conference on System Modeling and Advancement in Research Trends (SMART), 2019.  
  Presents a neural-network-based approach for genre classification in Bangla music.

### Undegrad Thesis
- **an-Eye: Safe Navigation in Footpath for Visually Impaired Using Computer Vision Techniques**  
  *Md Afif Al Mamun* (Undergraduate Thesis), 2020.  
  A computer-vision-driven assistive navigation system designed to enhance mobility for visually impaired individuals.
